...
pub fn buy_shorts1(
    ...
    let r0 = ctx.accounts.ccb1_token.amount as f64;
    let r1 = ctx.accounts.ccs0_token.amount as f64;
    let r2 = (ctx.accounts.ccb1_token.amount + amount) as f64;
    let s0_to_owner = (r1 *
        (1.0 - (r0 / r2).powf(ctx.accounts.mint_authority.smod))
    ).floor() as u64;
    assert_eq!(s0_to_owner > 0, true);

    let r3 = (ctx.accounts.ccs0_token.amount - s0_to_owner) as f64;
    let cc_from_owner = (r0 *
    ...
    let cr1 = (ctx.accounts.cc_token.amount
        - ctx.accounts.ccb1_token.amount) as f64;
    let cr2 = r0 + cc_from_owner as f64;
      // (cr1 - cc_to_burn) * cr2 == cr1 * cr0
    let cc_to_burn = (cr1 - cr1 * r0 / cr2).floor() as u64;
    ...
    token::transfer(
        CpiContext::new_with_signer(
            ...
            token::Transfer {
                ...
                to: ctx.accounts.owner_ccs0_token.to_account_info(),
