...
pub fn crank3(
    ...
    let f0 = (ctx.accounts.cc_token.amount
        - ctx.accounts.ccb1_token.amount) as f64;
        // * ctx.accounts.ccb1_token.amount as f64).sqrt();
    let mut x0 = f0.floor() as u64;
    if x0 < ctx.accounts.ccb1_token.amount {
        x0 = ctx.accounts.ccb1_token.amount;
    }
    let x1 = ctx.accounts.cc_token.amount
        - ctx.accounts.ccb1_token.amount - f0.ceil() as u64;
    if x1 > 0 {
        token::burn(
    ...
    token::burn(
        ...
        ), ctx.accounts.ccb1_token.amount,
    )?;
    ...
    ctx.accounts.mint_authority.maturity_state = 0;
    Ok(())
}
